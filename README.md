...
<img src="https://latex.codecogs.com/svg.latex?L_{lc}\left&space;(&space;\Theta&space;\right&space;)=&space;\frac{1}{N}\sum_{t=2}^{T}\sum_{x}^{N}max\left&space;(&space;y_{t}-y_{t-1},0&space;\right&space;)" title="L_{lc}\left ( \Theta \right )= \frac{1}{N}\sum_{t=2}^{T}\sum_{x}^{N}max\left ( y_{t}-y_{t-1},0 \right )" />
