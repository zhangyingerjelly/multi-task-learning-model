...
def model_construct():
    ...
    # 搭建后方四个 subnet
    d_cvr1 = Dense(128, activation='relu')(input_concatenate_layer)
    d_cvr1 = Dense(64, activation='relu')(d_cvr1)
    d_cvr1 = Dense(32, activation='relu')(d_cvr1)
    d_cvr1 = Dense(1, activation='sigmoid')(d_cvr1)
    ...
    d_click = Dense(1, activation='sigmoid')(d_click)

    d_apply=Lambda(lambda x: tf.multiply(x[0],x[1]))([d_cvr1,d_click])
    print("d_apply:",d_apply)

    d_cvr2 = Dense(128, activation='relu')(input_concatenate_layer)
    d_cvr2 = Dense(64, activation='relu')(d_cvr2)
    d_cvr2 = Dense(32, activation='relu')(d_cvr2)
    d_cvr2 = Dense(1, activation='sigmoid')(d_cvr2)
    d_credit=Lambda(lambda x: tf.multiply(x[0],x[1]))([d_cvr2,d_apply])

    d_cvr3 = Dense(128, activation='relu')(input_concatenate_layer)
    d_cvr3 = Dense(64, activation='relu')(d_cvr3)
    d_cvr3 = Dense(32, activation='relu')(d_cvr3)
    d_cvr3 = Dense(1, activation='sigmoid')(d_cvr3)
    d_activate=Lambda(lambda x: tf.multiply(x[0],x[1]))([d_cvr3,d_credit])

    # concat 这些带标签的节点作为输出，方便计算loss
    output_layers=[d_click,d_apply,d_credit,d_activate] 
