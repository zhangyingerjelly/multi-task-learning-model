...
class MMoE(Layer):  
    ...
    def __init__(self,
        ...
        #self.gate_kernel_initializer=tf.keras.initializers.VarianceScaling(
    #scale=1.0, mode='fan_in', distribution='normal', seed=seed)
        ...
        # Keras parameter
        self.input_spec = InputSpec(min_ndim=2)  #每个层都应公开(如果适用)一个input_spec属性：InputSpec的实例列表(每个输入张量一个).
    ...
    def build(self, input_shape):  ## Create the state of the layer (weights)
        ...
        #print("input_dimension:",input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        expert_outputs = K.tf.tensordot(a=inputs, b=self.expert_kernels, axes=1) #a的最后一个维度与b的第一个维度矩阵相乘，然后将结果累加求和
        # Add the bias term to the expert weights if necessary
        if self.use_expert_bias:
            print("expert_outputs:",expert_outputs)
            print("self.expert_bias:",self.expert_bias)
        ...
        for gate_output in gate_outputs:
            # gate_output shape (?, 8)
            expanded_gate_output = K.expand_dims(gate_output, axis=1) #expanded_gate_output shape (?, 1, 8)
            #weighted_expert_output shape (?, 4, 8)
            print("expert_outputs:",expert_outputs)
            print(K.repeat_elements(expanded_gate_output, self.units, axis=1))
            weighted_expert_output = expert_outputs * K.repeat_elements(expanded_gate_output, self.units, axis=1)
            
            final_outputs.append(K.sum(weighted_expert_output, axis=2)) #(len=2),(?,4)
